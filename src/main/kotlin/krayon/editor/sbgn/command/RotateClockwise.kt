...
object RotateClockwise: SbgnCommand("ROTATE_CLOCKWISE") {
    ...
    override fun execute(param: Any?) {
        ...
        getNodes(param).let { nodes ->
            graph.beginEdit(id, nodes + nodes.map { it.tag }).use {
                RotateTransformation.rotate(graph, nodes, RotateTransformation.RotationDirection.CLOCKWISE, graphComponent.createRenderContext())
            }
        }
